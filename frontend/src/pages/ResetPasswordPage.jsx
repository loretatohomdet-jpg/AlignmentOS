...
export default function ResetPasswordPage() {
  ...
  const shell = (children) => (
    <div className={`${type.page} antialiased`}>
      <SiteMarketingHeader />
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-16 font-sans">
  ...
  if (!token && !success) {
    return shell(
      <div className="text-center">
        <h1 className={authHeadingClass}>Invalid reset link</h1>
        <p className={`${authLeadClass} max-w-md mx-auto`}>
          This link is missing or invalid. Request a new one from the sign-in page.
        </p>
        <NavLink to="/forgot-password" className="mt-6 inline-block text-sm font-medium text-alignment-accent hover:underline">
          Forgot password
        </NavLink>
      </div>
    );
  ...
  if (success) {
    return shell(
      <div className="text-center">
        <h1 className={authHeadingClass}>Password updated</h1>
        <p className={`${authLeadClass} max-w-md mx-auto`}>You can now sign in with your new password.</p>
        <NavLink to="/login" className={`${pillPrimary} mt-8`}>
          Sign in
        </NavLink>
      </div>
    );
  ...
  return shell(
    <div className="animate-fade-in">
      <div className="text-center mb-10">
        ...
        <p className={`${authLeadClass} max-w-md mx-auto`}>Enter your new password below.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button type="submit" disabled={loading} className={authPrimaryButtonMarketingClass}>
